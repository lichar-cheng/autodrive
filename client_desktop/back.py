...
def occupancy_grid_from_cells(occupied_cells: list[dict], free_cells: list[dict], resolution: float) -> dict:
    ...
    if not all_cells:
        return ensure_grid_runtime_state({"width": 1, "height": 1, "resolution": resolution, "origin": {"x": 0.0, "y": 0.0}, "data": [-1]})
    min_ix = min(int(cell.get("ix", 0)) for cell in all_cells)
    max_ix = max(int(cell.get("ix", 0)) for cell in all_cells)
    min_iy = min(int(cell.get("iy", 0)) for cell in all_cells)
    max_iy = max(int(cell.get("iy", 0)) for cell in all_cells)
    ...
    for cell in occupied_cells:
        set_cell(cell, 100)
    return ensure_grid_runtime_state({
        "width": width,
        "height": height,
...
def occupancy_grid_counts(grid: dict) -> dict[str, int]:
    ...
    if isinstance(cached, dict):
        return {
            "obstacle": int(cached.get("obstacle", 0) or 0),
            "safe": int(cached.get("safe", 0) or 0),
            "unknown": int(cached.get("unknown", 0) or 0),
        }
    data = list(grid.get("data", []))
    counts = {
        "obstacle": sum(1 for value in data if int(value) >= 50),
        "safe": sum(1 for value in data if int(value) == 0),
...
def write_slam_archive(path: str | Path, manifest: dict, grid: dict, pcd_file: dict[str, Any] | None) -> Path:
    ...
    archive_manifest.pop("occupancy_grid", None)
    archive_manifest["version"] = "slam.v4"
    archive_manifest["map_storage"] = "occupancy_grid"
    archive_manifest["occupancy_grid"] = grid_manifest(grid)
    ...
    with zipfile.ZipFile(target, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr("grid.bin", grid_to_bytes(grid))
        if isinstance(pcd_file, dict):
...
def read_slam_archive(path: str | Path) -> tuple[dict, dict, dict[str, Any] | None]:
    ...
    grid["data"] = bytes_to_grid_data(raw)
    ensure_grid_runtime_state(grid)
...
class DesktopClient:
    def __init__(self) -> None:
        ...
        self.server_grid = {"active": False, "resolution": 0.0, "occupied_cells": [], "free_cells": [], "data": [], "width": 0, "height": 0, "origin": {"x": 0.0, "y": 0.0}, "stamp": 0.0}
        self.map_grid = {"active": False, "width": 0, "height": 0, "resolution": 0.08, "origin": {"x": 0.0, "y": 0.0}, "data": []}
        self.map_photo = None
    ...
    def sync_scan_badges(self) -> None:
        ...
        if grid.get("active"):
            counts = occupancy_grid_counts(grid)
            occ = int(counts["obstacle"])
            free = int(counts["safe"])
        else:
    ...
    def active_grid(self) -> dict:
        ...
        if bool(loaded_grid.get("active")):
            return ensure_grid_runtime_state(loaded_grid)
        if self.should_use_server_grid():
            server_grid = getattr(self, "server_grid", {}) or {}
            if isinstance(server_grid.get("data"), list) and int(server_grid.get("width", 0) or 0) > 0:
                ensure_grid_runtime_state(server_grid)
    ...
    def set_active_grid_value(self, grid_x: int, grid_y: int, value: int) -> None:
        ...
        counts = grid.get("_counts") if isinstance(grid.get("_counts"), dict) else None
        if counts is not None:
            counts[occupancy_bucket(old_value)] = max(0, int(counts.get(occupancy_bucket(old_value), 0) or 0) - 1)
            counts[occupancy_bucket(new_value)] = int(counts.get(occupancy_bucket(new_value), 0) or 0) + 1
        grid["_revision"] = int(grid.get("_revision", 0) or 0) + 1
        if bool(getattr(self, "map_grid", {}).get("active")):
            self.map_grid["data"] = data
            self.map_grid["_counts"] = grid.get("_counts")
            self.map_grid["_revision"] = grid.get("_revision", 0)
        elif bool(getattr(self, "server_grid", {}).get("active")):
            self.server_grid["data"] = data
            self.server_grid["_counts"] = grid.get("_counts")
            self.server_grid["_revision"] = grid.get("_revision", 0)
        self.map_photo_key = None
        self.mark_canvas_dirty()
    ...
    def active_occupancy_cells(self) -> list[dict]:
        grid = self.active_grid()
        if grid.get("active"):
            return self.grid_cells(grid, occupied=True)
        if self.should_use_server_grid():
    ...
    def active_free_cells(self) -> list[dict]:
        grid = self.active_grid()
        if grid.get("active"):
            return self.grid_cells(grid, occupied=False)
        if self.should_use_server_grid():
    ...
    def grid_cells(self, grid: dict, occupied: bool) -> list[dict]:
        ...
        for index, value in enumerate(grid.get("data", [])):
            value = int(value)
            if occupied and value < 50:
                continue
            if not occupied and value != 0:
                continue
            row = index // width
            col = index % width
            cell = {"ix": origin_ix + col, "iy": origin_iy + row, "hits": 3}
            if occupied:
                cell["intensity"] = 1.0
            cells.append(cell)
        return cells
    ...
    def active_map_fence_xy(self) -> list[dict[str, float]]:
        ...
        if grid.get("active"):
            ...
            if width > 0 and height > 0:
                resolution = max(0.02, float(grid.get("resolution", self.active_voxel_size())))
    ...
    def update_server_grid(self, payload: dict[str, Any], stamp: float) -> None:
        ...
        self.server_grid = ensure_grid_runtime_state({
            "active": True,
            "resolution": resolution,
        ...
        if grid.get("active"):
            ...
            for index, value in enumerate(grid.get("data", [])):
                if int(value) >= 50:
                    row = index // width
                    col = index % width
                    lookup[(origin_ix + col, origin_iy + row)] = {"ix": origin_ix + col, "iy": origin_iy + row, "hits": 3, "intensity": 1.0}
            return lookup
    ...
    def current_occupancy_grid(self) -> dict:
        ...
        if grid.get("active"):
            return {
                ...
                "resolution": max(0.02, float(grid.get("resolution", self.active_voxel_size()))),
                "origin": dict(grid.get("origin", {"x": 0.0, "y": 0.0})),
                "data": [int(value) for value in grid.get("data", [])],
            }
    ...
    def erase_radius(self, world_x: float, world_y: float) -> None:
        ...
        grid = self.active_grid()
        if grid.get("active"):
            resolution = max(0.02, float(grid.get("resolution", self.scan["voxel"])))
            radius_cells = max(1, round(radius / resolution))
            cx, cy = self.world_to_active_grid_cell(world_x, world_y, grid)
            for dx in range(-radius_cells, radius_cells + 1):
                for dy in range(-radius_cells, radius_cells + 1):
                    if math.hypot(dx, dy) * resolution <= radius:
                        self.set_active_grid_value(cx + dx, cy + dy, 0)
            self.sync_scan_badges()
            return
    ...
    def draw_obstacle_line(self, start: tuple[float, float], end: tuple[float, float]) -> None:
        grid = self.active_grid()
        if grid.get("active"):
            sx, sy = self.world_to_active_grid_cell(start[0], start[1], grid)
            ex, ey = self.world_to_active_grid_cell(end[0], end[1], grid)
            steps = max(abs(ex - sx), abs(ey - sy), 1)
            for step in range(steps + 1):
                t = step / steps
                self.set_active_grid_value(round(sx + (ex - sx) * t), round(sy + (ey - sy) * t), 100)
            return
    ...
    def active_map_world_bounds(self, include_robot: bool = False) -> tuple[float, float, float, float] | None:
        grid = self.active_grid()
        if grid.get("active"):
            resolution = max(0.02, float(grid.get("resolution", getattr(self, "scan", {}).get("voxel", 0.08))))
            origin = grid.get("origin") if isinstance(grid.get("origin"), dict) else {}
    ...
    def draw_cells(self) -> None:
        grid = self.active_grid()
        if grid.get("active"):
            self.draw_occupancy_grid_image(grid)
            return
        voxel = self.active_voxel_size()
    ...
    def draw_occupancy_grid_image(self, grid: dict) -> None:
        ...
        left, bottom = self.screen_to_world(0, canvas_height)
        right, top = self.screen_to_world(canvas_width, 0)
        if right < left:
            left, right = right, left
        if top < bottom:
            bottom, top = top, bottom
        grid_right = origin_x + width * resolution
        grid_top = origin_y + height * resolution
        visible_left = max(left, origin_x)
        visible_right = min(right, grid_right)
        visible_bottom = max(bottom, origin_y)
        visible_top = min(top, grid_top)
        if visible_left >= visible_right or visible_bottom >= visible_top:
            return
        col_start = max(0, min(width, math.floor((visible_left - origin_x) / resolution)))
        col_end = max(col_start + 1, min(width, math.ceil((visible_right - origin_x) / resolution)))
        row_start = max(0, min(height, math.floor((visible_bottom - origin_y) / resolution)))
        row_end = max(row_start + 1, min(height, math.ceil((visible_top - origin_y) / resolution)))
        display_width = max(1, round((col_end - col_start) * resolution * self.view["scale"]))
        display_height = max(1, round((row_end - row_start) * resolution * self.view["scale"]))
        key = (int(grid.get("_revision", 0) or 0), col_start, col_end, row_start, row_end, display_width, display_height)
        if self.map_photo_key != key:
            crop_width = col_end - col_start
            crop_height = row_end - row_start
            source_cols = [col_start + (col * crop_width) // display_width for col in range(display_width)]
            body = bytearray(display_width * display_height * 3)
            out = 0
            for row in range(display_height):
                source_row = row_end - 1 - ((row * crop_height) // display_height)
                offset = source_row * width
                for source_col in source_cols:
                    value = int(data[offset + source_col])
                    if value >= 50:
                        body[out:out + 3] = b"\x0c\x0f\x12"
                    elif value == 0:
                        body[out:out + 3] = b"\xff\xff\xff"
                    else:
                        body[out:out + 3] = b"\xb8\xbe\xc3"
                    out += 3
            ppm = f"P6\n{display_width} {display_height}\n255\n".encode("ascii") + bytes(body)
            self.map_photo = tk.PhotoImage(master=self.root, data=ppm, format="PPM")
            self.map_photo_key = key
    ...
    def build_pgm_export(self, manifest: dict, grid_payload: dict, resolution: float, padding_cells: int = 0) -> dict:
        ...
        data = [int(value) for value in grid_payload.get("data", [])]
        if width <= 0 or height <= 0 or len(data) != width * height:
        ...
        origin_meta = grid_payload.get("origin") if isinstance(grid_payload.get("origin"), dict) else {}
    ...
    def prepare_inspector_exports(self) -> None:
        ...
        self.inspector["yaml"] = self.build_yaml_export(
            str(self.inspector.get("file") or "map_export"),
            resolution,
            pgm["origin"],
        )
        self.inspector["json"] = json.dumps(
            build_export_json_manifest(manifest),
            ensure_ascii=False,
            indent=2,
        )
    ...
    def save_stcm(self) -> None:
        ...
        grid_counts = occupancy_grid_counts(grid)
        bundle = {
            "version": "slam.v4",
            ...
            "scan_summary": {
                ...
                "obstacleCells": grid_counts["obstacle"],
                "safeCells": grid_counts["safe"],
        ...
        self.sync_scan_badges()
        self.set_inspector_bundle_state(Path(target).name, manifest, grid)
    ...
    def load_stcm(self) -> None:
        ...
        self.apply_stcm(file_name, manifest, grid, pcd_file=pcd_file)
        self.set_inspector_bundle_state(file_name, manifest, grid)
    ...
    def apply_stcm(self, file_name: str, manifest: dict, grid: dict, pcd_file: dict[str, Any] | None = None) -> None:
        ...
        self.scan["voxel"] = max(0.02, float(grid.get("resolution", config["voxel_size"])))
        self.map_grid = ensure_grid_runtime_state({"active": True, **grid})
        self.map_photo_key = None
        self.poi_nodes = []
        self.poi_seed = 1
        ...
        counts = occupancy_grid_counts(grid)
        self.stcm_summary = {"file": file_name, "mapSource": manifest.get("map_source", "unknown"), "radarPoints": counts["obstacle"], "poiCount": len(self.poi_nodes), "pathCount": len(self.path_segments), "hasBrowserOccupancy": False, "restoredFreeCells": counts["safe"]}
        if isinstance(pcd_file, dict):
    ...
    def export_inspector_file(self, kind: str) -> None:
        ...
        if kind == "zip":
            ...
            if not path:
                return
            self.prepare_inspector_exports()
